...
def _get_triplet_test_mask(labels,test,device='cpu'):
    """
    Return a 3D mask where mask[a, p, n] is True if the triplet (a, p, n) contain test samples
    ...
    j_include_test =  (torch.unsqueeze(labels, 0) == torch.unsqueeze(test, 1))
    i_include_test =  (torch.unsqueeze(labels, 1) == torch.unsqueeze(test, 0))
    i_j_include_test = j_include_test | i_include_test
    i_j_k_include_test= (torch.unsqueeze(i_j_include_test, 2) | torch.unsqueeze(i_j_include_test, 0))
    num_triplet = torch.nonzero((i_j_k_include_test==True)).shape[0]

    return i_j_k_include_test
...
def _get_triplet_mask(labels, device='cpu'):
    ...
    """ since each sample in longitudinal dissimilarity of temporal microbiome data has a different label
    We do not require anchor and positive have same labele
    here we construct triplets if |label[i] - label[j]|<  |label[i] - label[k]|"""
    
    label_difference_abs = abs( (torch.unsqueeze(labels, 1) - torch.unsqueeze(labels, 0)) )
    i_subtract_j = torch.unsqueeze(label_difference_abs , 2)
    i_subtract_k = torch.unsqueeze(label_difference_abs , 1)
    val= i_subtract_j < i_subtract_k  
    # Combine the two masks 
    mask = (distinct_indices.to(device).bool() & val) 
   
    num_triplet = torch.nonzero((mask==True)).shape[0]
...
def online_mine_all(labels, embeddings, margin,dist_model, test= '',squared=False, device='cpu'):
    ...
    Args:
        ...
        dist_model : choose which distance to ues , 'cos' represent cosine distance ,  'euclidean distances'
        test: list, choose one or two samples as unknown testing data from temporal points ,
                    then construct test triplets that conntain testing data
    ...
    # if 'test' is not empty , construct test triplets
    # Note that in this experiment, test triplets refer to those that contain testing samples specified by the 'test'
    if(test != ''):
        mask_all = torch.full(mask.shape,False,device=device)
        for item in test: 
            mask_test= (_get_triplet_test_mask(labels,item, device=device))
            mask_tmp = mask & mask_test
